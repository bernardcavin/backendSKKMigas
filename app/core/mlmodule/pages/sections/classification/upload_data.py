...
import flask
import uuid
from app.core.mlmodule.pages.sections.classification.utils import session_delete_file, parse_contents, render_upload_header, reset_button, continue_button
...
def layout():
    ...
    layout = dmc.Stack(
        [
            html.Div(
                upload_button,
                id='classification-upload-header'
            ),
            html.Div(
                id='classification-output-data',
            ),
            dmc.Group(
                [
                    reset_button,
...
@callback(
    Output('classification-output-data', 'children', allow_duplicate=True),
    Output('classification-upload-header', 'children', allow_duplicate=True),
...
def upload_data_processing(contents, filename):
    if contents is not None:
        ...
        try:
            ...
            upload_header = render_upload_header(filename, 'classification-rawData')
...
@callback(
    Output('classification-output-data', 'children'),
    Output('classification-upload-header', 'children'),
    Input('classification-rawData-remove-data', 'n_clicks'),
)
