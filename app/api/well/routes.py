...
async def create_well(well: schemas.CreatePlanWell, db: Session = Depends(get_db), user = Depends(get_current_user)):
    try:
        well_id = crud.create_well(db, well, user)
        return create_api_response(success=True, message="Well created successfully", data={"id": well_id})
